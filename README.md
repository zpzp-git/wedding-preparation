...
打开 <http://localhost:3000>。首次启动会运行数据库迁移，并初始化中国婚礼默认项目树；资源分类会从采用“方案对比”的项目自动生成。执行 `npm run db:examples` 可在空业务数据库中加入一组可编辑的商家、宾客、候选方案和方案快照示例；当前本地数据库已加入这组示例。点击左侧新人姓名可设置婚礼日期、地点和预算。后端健康检查地址为 <http://localhost:3000/api/health>。
